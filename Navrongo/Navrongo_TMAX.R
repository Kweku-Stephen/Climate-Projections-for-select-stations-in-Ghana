...
dir(
	...
	) -> Navrongo_TMAX_RCP26_Corrected_uncorrected
...
# Data Reshaping ####
Navrongo_TMAX_RCP26 <- lapply(
	Navrongo_TMAX_RCP26_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Navrongo_TMAX_RCP26_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmax")]
		} else{
			data[ ,"Tmax"]
		}
...
split(
	Navrongo_TMAX_RCP26[ ,-1],
	format(Navrongo_TMAX_RCP26[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Navrongo_MeanTMAX_RCP26
...
dir(
	...
	) -> Navrongo_TMAX_RCP45_Corrected_uncorrected
...
# Data Reshaping ####
Navrongo_TMAX_RCP45 <- lapply(
	Navrongo_TMAX_RCP45_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Navrongo_TMAX_RCP45_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmax")]
		} else{
			data[ ,"Tmax"]
		}
...
# SPLIT-APPLY-COMBINE METHOD RETURNS ALL NAs AS ZEROs
split(
	Navrongo_TMAX_RCP45[ ,-1],
	format(Navrongo_TMAX_RCP45[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Navrongo_MeanTMAX_RCP45
...
dir(
	...
	) -> Navrongo_TMAX_RCP85_Corrected_uncorrected
...
split(
	Navrongo_TMAX_RCP85[ ,-1],
	format(Navrongo_TMAX_RCP85[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Navrongo_MeanTMAX_RCP85
...
`Ensembles of scenarios TMAX Navrongo` <- data.frame(
	...
	RCP85 = Navrongo_MeanTMAX_RCP85[ ,"Ensemble"]
) |> 
	subset(Date >= 1980)
