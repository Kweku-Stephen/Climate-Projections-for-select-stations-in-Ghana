...
dir(
	...
	) -> capecoast_RR_RCP26_Corrected_uncorrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> capecoast_RR_RCP26_Corrected_unduplicated
...
# Data Reshaping ####
capecoast_RR_RCP26 <- lapply(
	capecoast_RR_RCP26_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(capecoast_RR_RCP26_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	capecoast_RR_RCP26[ ,-1],
	format(capecoast_RR_RCP26[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> capecoast_RainFallTotal_RCP26
...
dir(
	...
	) -> capecoast_RR_RCP45_Corrected_uncorrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> capecoast_RR_RCP45_Corrected_unduplicated
...
# Data Reshaping ####
capecoast_RR_RCP45 <- lapply(
	capecoast_RR_RCP45_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(capecoast_RR_RCP45_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
# SPLIT-APPLY-COMBINE METHOD RETURNS ALL NAs AS ZEROs
split(
	capecoast_RR_RCP45[ ,-1],
	format(capecoast_RR_RCP45[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) # summing only NAs returns 0 
	) |> . =>
	do.call(rbind, .) -> capecoast_RainFallTotal_RCP45
...
dir(
	...
	) -> capecoast_RR_RCP85_Corrected_uncorrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> capecoast_RR_RCP85_Corrected_unduplicated
...
# Data Reshaping ####
capecoast_RR_RCP85 <- lapply(
	capecoast_RR_RCP85_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(capecoast_RR_RCP85_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	capecoast_RR_RCP85[ ,-1],
	format(capecoast_RR_RCP85[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> capecoast_RainFallTotal_RCP85
...
`Ensembles of scenarios capecoast` <- data.frame(
	Date = as.numeric(rownames(capecoast_RainFallTotal_RCP26)),
	RCP26 = capecoast_RainFallTotal_RCP26[ ,"Ensemble"],
	RCP45 = capecoast_RainFallTotal_RCP45[ ,"Ensemble"],
	RCP85 = capecoast_RainFallTotal_RCP85[ ,"Ensemble"]
) |> 
	subset(Date >= 1980)

# Observed, historicals of scenarios plus emsemble of historical of scenarios ####
`observed + Hist_scenarios capecoast` = data.frame(
	observed = read.csv(
		"C:\\Users\\pc\\OneDrive\\Documents\\Climate_of_Select_Stations\\Rainfall Total\\capecoast_prcp.csv",
		col.names = c("Year", "observed")
		#skip = 20
	),
	RCP26 = subset(`Ensembles of scenarios capecoast`, Date <= 2019)[ ,"RCP26"],
	RCP45 = subset(`Ensembles of scenarios capecoast`, Date <= 2019)[ ,"RCP45"],
	RCP85 = subset(`Ensembles of scenarios capecoast`, Date <= 2019)[ ,"RCP85"]
) |> . =>
	within(
		.,
		{
