...
dir(
	...
	) -> sefwi_bekua_RR_RCP26_Corrected_uncorrected
...
) -> sefwi_bekua_RR_RCP26_Corrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> sefwi_bekua_RR_RCP26_Corrected_unduplicated
...
sefwi_bekua_RR_RCP26_Corrected_unduplicated %<>% 
	lapply(
		#sefwi bekua_RR_RCP26_Corrected_unduplicated,
		\(data = "") {
...
# Data Reshaping ####
sefwi_bekua_RR_RCP26 <- lapply(
	sefwi_bekua_RR_RCP26_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(sefwi_bekua_RR_RCP26_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	sefwi_bekua_RR_RCP26[ ,-1],
	format(sefwi_bekua_RR_RCP26[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> sefwi_bekua_RainFallTotal_RCP26
...
dir(
	...
	) -> sefwi_bekua_RR_RCP45_Corrected_uncorrected
...
) -> sefwi_bekua_RR_RCP45_Corrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> sefwi_bekua_RR_RCP45_Corrected_unduplicated
...
# sefwi bekua_RR_RCP45_Corrected_unduplicated %<>%
# 	lapply(
# 		\(data = "") data[!duplicated(data[ ,"Date"]), ]
# 	)
...
# Data Reshaping ####
sefwi_bekua_RR_RCP45 <- lapply(
	sefwi_bekua_RR_RCP45_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(sefwi_bekua_RR_RCP45_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
	...
	data.frame(do.call(cbind, .)) |>
	setNames(c("Date", "MIROC5", "NOAA", "CNRM", "MPI")) |>  . =>
	within(
		.,
		{Ensemble = apply(.[ ,-1], 1, mean, na.rm = TRUE)}
	)
...
# SPLIT-APPLY-COMBINE METHOD RETURNS ALL NAs AS ZEROs
split(
	sefwi_bekua_RR_RCP45[ ,-1],
	format(sefwi_bekua_RR_RCP45[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) # summing only NAs returns 0 
	) |> . =>
	do.call(rbind, .) -> sefwi_bekua_RainFallTotal_RCP45
...
dir(
	...
	) -> sefwi_bekua_RR_RCP85_Corrected_uncorrected
...
) -> sefwi_bekua_RR_RCP85_Corrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> sefwi_bekua_RR_RCP85_Corrected_unduplicated
...
# Data Reshaping ####
sefwi_bekua_RR_RCP85 <- lapply(
	sefwi_bekua_RR_RCP85_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(sefwi_bekua_RR_RCP85_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	sefwi_bekua_RR_RCP85[ ,-1],
	format(sefwi_bekua_RR_RCP85[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> sefwi_bekua_RainFallTotal_RCP85
...
`Ensembles of scenarios sefwi bekua` <- data.frame(
	Date = as.numeric(rownames(sefwi_bekua_RainFallTotal_RCP26)),
	RCP26 = sefwi_bekua_RainFallTotal_RCP26[ ,"Ensemble"],
	RCP45 = sefwi_bekua_RainFallTotal_RCP45[ ,"Ensemble"],
	RCP85 = sefwi_bekua_RainFallTotal_RCP85[ ,"Ensemble"]
) |> 
	subset(Date >= 1980)

# Observed, historicals of scenarios plus emsemble of historical of scenarios ####
`observed + Hist_scenarios sefwi bekua` = data.frame(
	observed = read.csv(
		"C:\\Users\\pc\\OneDrive\\Documents\\Climate_of_Select_Stations\\Rainfall Total\\Sefwi_rr.csv",
		col.names = c("Year", "observed")
		#skip = 20
	),
	RCP26 = subset(`Ensembles of scenarios sefwi bekua`, Date <= 2021)[ ,"RCP26"],
	RCP45 = subset(`Ensembles of scenarios sefwi bekua`, Date <= 2021)[ ,"RCP45"],
	RCP85 = subset(`Ensembles of scenarios sefwi bekua`, Date <= 2021)[ ,"RCP85"]
) |> . =>
	within(
		.,
		{
