...
dir(
	...
	) -> Koforiduah_TMAX_RCP26_Corrected_uncorrected
...
) -> Koforiduah_TMAX_RCP26_Corrected
...
# Data Reshaping ####
Koforiduah_TMAX_RCP26 <- lapply(
	Koforiduah_TMAX_RCP26_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Koforiduah_TMAX_RCP26_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmax")]
		} else{
			data[ ,"Tmax"]
		}
	...
	setNames(c("Date", "CM5A", "NOAA", "MIROC5")) |>  . =>
	within(
		.,
...
split(
	Koforiduah_TMAX_RCP26[ ,-1],
	format(Koforiduah_TMAX_RCP26[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Koforiduah_MeanTMAX_RCP26
...
dir(
	...
	) -> Koforiduah_TMAX_RCP45_Corrected_uncorrected
...
) -> Koforiduah_TMAX_RCP45_Corrected
...
# Data Reshaping ####
Koforiduah_TMAX_RCP45 <- lapply(
	Koforiduah_TMAX_RCP45_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Koforiduah_TMAX_RCP45_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmax")]
		} else{
			data[ ,"Tmax"]
		}
...
# SPLIT-APPLY-COMBINE METHOD RETURNS ALL NAs AS ZEROs
split(
	Koforiduah_TMAX_RCP45[ ,-1],
	format(Koforiduah_TMAX_RCP45[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Koforiduah_MeanTMAX_RCP45
...
dir(
	...
	) -> Koforiduah_TMAX_RCP85_Corrected_uncorrected
...
) -> Koforiduah_TMAX_RCP85_Corrected
...
lapply(
	...
	var = "Tmax",
	Date = "Date"
) -> Koforiduah_TMAX_RCP85_Corrected_unduplicated
...
# Data Reshaping ####
Koforiduah_TMAX_RCP85 <- lapply(
	Koforiduah_TMAX_RCP85_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Koforiduah_TMAX_RCP85_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmax")]
		} else{
			data[ ,"Tmax"]
		}
...
split(
	Koforiduah_TMAX_RCP85[ ,-1],
	format(Koforiduah_TMAX_RCP85[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Koforiduah_MeanTMAX_RCP85
...
`Ensembles of scenarios TMAX Koforiduah` <- data.frame(
	Date = as.numeric(rownames(Koforiduah_MeanTMAX_RCP26)),
	RCP26 = Koforiduah_MeanTMAX_RCP26[ ,"Ensemble"],
	RCP45 = Koforiduah_MeanTMAX_RCP45[ ,"Ensemble"],
	RCP85 = Koforiduah_MeanTMAX_RCP85[ ,"Ensemble"]
) |> 
	subset(Date >= 1980)

# Observed, historicals of scenarios plus eNsemble of historical of scenarios ####
`observed + Hist_scenarios TMAX Koforiduah` = data.frame(
	observed = read.csv(
		"C:/Users/pc/OneDrive/Documents/Climate_of_Select_Stations/Koforidua Max Tmp.csv",
		col.names = c("Year", "observed")
	),
	RCP26 = subset(`Ensembles of scenarios TMAX Koforiduah`, Date <= 2021)[ ,"RCP26"],
	RCP45 = subset(`Ensembles of scenarios TMAX Koforiduah`, Date <= 2021)[ ,"RCP45"],
	RCP85 = subset(`Ensembles of scenarios TMAX Koforiduah`, Date <= 2021)[ ,"RCP85"]
) |> . =>
	within(
		.,
		{
