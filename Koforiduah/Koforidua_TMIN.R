...
dir(
	...
	) -> Koforiduah_TMIN_RCP26_Corrected_uncorrected
...
) -> Koforiduah_TMIN_RCP26_Corrected
...
lapply(
	...
	var = "Tmin",
	Date = "Date"
) -> Koforiduah_TMIN_RCP26_Corrected_unduplicated
...
# Data Reshaping ####
Koforiduah_TMIN_RCP26 <- lapply(
	Koforiduah_TMIN_RCP26_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Koforiduah_TMIN_RCP26_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmin")]
		} else{
			data[ ,"Tmin"]
		}
...
split(
	Koforiduah_TMIN_RCP26[ ,-1],
	format(Koforiduah_TMIN_RCP26[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Koforiduah_MeanTMIN_RCP26
...
dir(
	...
	) -> Koforiduah_TMIN_RCP45_Corrected_uncorrected
...
) -> Koforiduah_TMIN_RCP45_Corrected
...
lapply(
	...
	var = "Tmin",
	Date = "Date"
) -> Koforiduah_TMIN_RCP45_Corrected_unduplicated
...
# Koforiduah_TMAX_RCP45_Corrected_unduplicated %<>%
# 	lapply(
# 		\(data = "") data[!duplicated(data[ ,"Date"]), ]
# 	)
...
# Data Reshaping ####
Koforiduah_TMIN_RCP45 <- lapply(
	Koforiduah_TMIN_RCP45_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Koforiduah_TMIN_RCP45_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmin")]
		} else{
			data[ ,"Tmin"]
		}
...
# SPLIT-APPLY-COMBINE METHOD RETURNS ALL NAs AS ZEROs
split(
	Koforiduah_TMIN_RCP45[ ,-1],
	format(Koforiduah_TMIN_RCP45[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Koforiduah_MeanTMIN_RCP45
...
dir(
	...
	) -> Koforiduah_TMIN_RCP85_Corrected_uncorrected
...
) -> Koforiduah_TMIN_RCP85_Corrected
...
lapply(
	...
	var = "Tmin",
	Date = "Date"
) -> Koforiduah_TMIN_RCP85_Corrected_unduplicated
...
# Data Reshaping ####
Koforiduah_TMIN_RCP85 <- lapply(
	Koforiduah_TMIN_RCP85_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Koforiduah_TMIN_RCP85_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Tmin")]
		} else{
			data[ ,"Tmin"]
		}
...
split(
	Koforiduah_TMIN_RCP85[ ,-1],
	format(Koforiduah_TMIN_RCP85[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, mean, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Koforiduah_MeanTMIN_RCP85
...
`Ensembles of scenarios TMIN Koforiduah` <- data.frame(
	Date = as.numeric(rownames(Koforiduah_MeanTMIN_RCP26)),
	RCP26 = Koforiduah_MeanTMIN_RCP26[ ,"Ensemble"],
	RCP45 = Koforiduah_MeanTMIN_RCP45[ ,"Ensemble"],
	RCP85 = Koforiduah_MeanTMIN_RCP85[ ,"Ensemble"]
) |> 
	subset(Date >= 1980)

# Observed, historicals of scenarios plus eNsemble of historical of scenarios ####
`observed + Hist_scenarios TMIN Koforiduah` = data.frame(
	observed = read.csv(
		"C:/Users/pc/OneDrive/Documents/Climate_of_Select_Stations/Koforidua Min Tmp.csv",
		col.names = c("Year", "observed")
	),
	RCP26 = subset(`Ensembles of scenarios TMIN Koforiduah`, Date <= 2021)[ ,"RCP26"],
	RCP45 = subset(`Ensembles of scenarios TMIN Koforiduah`, Date <= 2021)[ ,"RCP45"],
	RCP85 = subset(`Ensembles of scenarios TMIN Koforiduah`, Date <= 2021)[ ,"RCP85"]
) |> . =>
	within(
		.,
		{
...
gridExtra::grid.arrange(
	ggplot(data = `Ensembles of scenarios Koforiduah`,aes(x = Date)) +
		geom_line(data = `observed + Hist_scenarios Koforiduah`, aes(x = Year, y = RCP26, col = "Historical"), lwd = 1) +
		geom_line(data = `observed + Hist_scenarios Koforiduah`, aes(x = Year, y = RCP45, col = "Historical"), lwd = 1) +
		geom_line(data = `observed + Hist_scenarios Koforiduah`, aes(x = Year, y = RCP85, col = "Historical"), lwd = 1) +
		geom_line(data = `observed + Hist_scenarios Koforiduah`, aes(x = Year, y = `Ensemble of hist scenarios`, col = "Ensemble"), lwd = 1) +
		geom_line(data = `observed + Hist_scenarios Koforiduah`, aes(x = Year, y = observed, col = "Observed"), lwd = 2)+
		geom_line(data = subset(`Ensembles of scenarios Koforiduah`, Date >= 2022), 
...
dev.copy(png, filename = "Plots_Outputs/Projected Climate of Koforiduah.png", width = 1500, height = 900)
dev.off()
