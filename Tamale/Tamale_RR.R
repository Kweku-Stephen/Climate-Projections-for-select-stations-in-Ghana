...
dir(
	...
	) -> Tamale_RR_RCP26_Corrected_uncorrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> Tamale_RR_RCP26_Corrected_unduplicated
...
# Data Reshaping ####
Tamale_RR_RCP26 <- lapply(
	Tamale_RR_RCP26_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Tamale_RR_RCP26_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	Tamale_RR_RCP26[ ,-1],
	format(Tamale_RR_RCP26[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Tamale_RainFallTotal_RCP26
...
dir(
	...
	) -> Tamale_RR_RCP45_Corrected_uncorrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> Tamale_RR_RCP45_Corrected_unduplicated
...
# Data Reshaping ####
Tamale_RR_RCP45 <- lapply(
	Tamale_RR_RCP45_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Tamale_RR_RCP45_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	Tamale_RR_RCP45[ ,-1],
	format(Tamale_RR_RCP45[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) # summing only NAs returns 0 
	) |> . =>
	do.call(rbind, .) -> Tamale_RainFallTotal_RCP45
...
dir(
	...
	) -> Tamale_RR_RCP85_Corrected_uncorrected
...
lapply(
	...
	var = "Rain",
	Date = "Date"
) -> Tamale_RR_RCP85_Corrected_unduplicated
...
# Data Reshaping ####
Tamale_RR_RCP85 <- lapply(
	Tamale_RR_RCP85_Corrected_unduplicated,
	
	\(data = "") {
		if(identical(Tamale_RR_RCP85_Corrected_unduplicated[[1]], data)){
			data[ ,c("Date", "Rain")]
		} else{
			data[ ,"Rain"]
		}
...
split(
	Tamale_RR_RCP85[ ,-1],
	format(Tamale_RR_RCP85[ ,"Date"], format = "%Y")
) |> 
	lapply(
		\(data = "") sapply(data, sum, na.rm = TRUE) 
	) |> . =>
	do.call(rbind, .) -> Tamale_RainFallTotal_RCP85
...
`Ensembles of scenarios Tamale` <- data.frame(
	Date = as.numeric(rownames(Tamale_RainFallTotal_RCP26)),
	RCP26 = Tamale_RainFallTotal_RCP26[ ,"Ensemble"],
	RCP45 = Tamale_RainFallTotal_RCP45[ ,"Ensemble"],
	RCP85 = Tamale_RainFallTotal_RCP85[ ,"Ensemble"]
) |> 
	subset(Date >= 1980)

# Observed, historicals of scenarios plus emsemble of historical of scenarios ####
`observed + Hist_scenarios Tamale` = data.frame(
	observed = read.csv(
		"C:/Users/pc/OneDrive/Documents/R_PROJECTS/Tamale/Annual Rainfall of Tamale(1960-2016).csv",
		col.names = c("Year", "observed"),
		skip = 20
	) |> . =>
		transform(., Year = as.Date(.[ ,"Year"], "%m/%d/%Y")),
	RCP26 = subset(`Ensembles of scenarios Tamale`, Date <= 2016)[ ,"RCP26"],
	RCP45 = subset(`Ensembles of scenarios Tamale`, Date <= 2016)[ ,"RCP45"],
	RCP85 = subset(`Ensembles of scenarios Tamale`, Date <= 2016)[ ,"RCP85"]
) |> . =>
	within(
		.,
		{
